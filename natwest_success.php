...
if ($type === "user") {
    // Check if the token is set
        $sql_payment = "UPDATE orders SET payment_status='Success', payment_method='Natwest' WHERE userid='$userid' AND orderid='$orderid'";
        ...
        if ($payment_query) {
            ...
            try {
                ...
                $mail->Subject = 'Order Placed - OrderId #' . $orderid;
        ...
        if (isset($_GET['response'])) {
            ...
            if ($payment_query) {
                ...
                try {
                    ...
                    $mail->setFrom($smtp_username, 'Equipride');
                    $mail->addAddress($order_email, 'Recipient Name');
                    $mail->Subject = 'Order Success - Order #' . $orderid;

...
$user_type_sql = "SELECT type FROM orders WHERE orderid='$orderid'";
...
if ($user_type_value['type'] === "user") {
    if (isset($token)) {
        $sql_payment = "UPDATE orders SET payment_status='Success', payment_method='PayPal' WHERE userid='$userid' AND orderid='$orderid'";
        ...
        ('$payer_type','$token','$userid','$order_email','$total_order_amount','GBP','$payment_method','Failed')";
        ...
        if ($payment_query) {
            ...
            try {
                ...
                $mail->Subject = 'Payment Success - Order #' . $orderid;
                $mail->Body = $email_body_for_user;
            ...
            if (mysqli_num_rows($query) > 0) {
                ...
                $mail2->Subject = 'Payment Success - Order #' . $orderid;
    ...
    if (isset($token)) {
        ...
        if ($payment_query) {
            ...
            try {
                ...
                $mail->addAddress($order_email, 'Recipient Name');
                $mail->Subject = 'Payment Success - Order #' . $orderid;
                $mail->Body = $email_body_for_guest;// Customize the email content

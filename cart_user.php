...
if (isset($_POST['submit'])) {
    ...
    $user_details_sql = "SELECT * FROM register WHERE userid='$userid'";
    ...
    if ($user_details = mysqli_fetch_assoc($user_details_query)) {
        ...
        try {
            ...
            while ($cart_item = mysqli_fetch_assoc($cart_items_query)) {
                ...
                VALUES ('$orderid', '{$user_details['userid']}', '{$user_details['name']}', '{$user_details['email']}', '{$user_details['phone']}', '{$user_details['address']}', '{$user_details['localty']}', '{$user_details['pincode']}', '{$user_details['country']}', 'shipping_address', '$product_name', '$price', '$size', '$quantity', '$total', 'null', 'processing','user')";
...
//   die();
    ...
    ?>
        <script type="text/javascript">
          window.location = "checkout_user.php?id=<?php echo $id ?>&&product_id=<?php echo $product_id ?>&&userid=<?php echo $userid ?>&&orderid=<?php echo $orderid; ?>";
        </script>
                                    ...
                                    <div class="row">
                                        <div class="col-lg-12"><br>
                                                    ...
                                                    <div class="ec-cart-summary-bottom">
                                                        <div class="ec-cart-summary">
                                                            ...
                                                            <input type="hidden" name="amount" class="totalAmount" id="totalAmount" value="">
                                                                ...
                                                                <span class="text-right totalAmount" value="£"></span>
                                                            </div>
                                                        </div>
                                            ...
                                            <div class="ec-cart-update-bottom">
                                                <a href="full_category_user.php">Continue Shopping</a>
                ...
                <div class="col-lg-3 col-md-6 col-sm-6 col-xs-6 mb-6 pro-gl-content">
                    <div class="ec-product-inner">
                        ...
                        <div class="ec-pro-content">
                            <h5 class="ec-pro-title"><a href="product_details_user.php?id=<?php echo $pc['id'] ?>&&product_id=<?php echo $pc['product_id'] ?>"><?php echo $pc['name'] ?></a></h5>
...
function calc(id, counter) {
  ...
  <script>
    // Function to update the page based on stored state
function updatePageFromStorage() {
    var selectedColor = localStorage.getItem('selectedColor');

    if (selectedColor) {
        // Set the selected color and trigger the change event
        document.querySelector('input[name="s_image"][value="' + selectedColor + '"]').checked = true;
        document.querySelector('input[name="s_image"]').dispatchEvent(new Event('change'));
    }
}
...
if (supportsLocalStorage()) {
    // Update the page from localStorage

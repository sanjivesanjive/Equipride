...
if (isset($_POST['stock'])) {
    ...
    // echo "INSERT INTO `stock`(`product_id`,`name`,`category`,`sub_category`,`quantity`,`color`,`price`,`dealer_price`,`type_1`,`status`) VALUES 
    // ('" . $id . "','" . $product_name . "','" . $category . "','" . $sub_category . "','" . $quantity . "','" . $size. "',
    // '" . $color . "','" . $price . "','" . $dealer_price . "','" . $type_1 . "','1')";
    // die();
  
    $query = "INSERT INTO `stock`(`product_id`,`product_name`,`category`,`sub_category`,`quantity`,`size`,`color`,`price`,`dealer_price`,`type_1`,`status`) VALUES 
    ('" . $id . "','" . $product_name . "','" . $category . "','" . $sub_category . "','" . $quantity . "','" . $size. "',
    '" . $color . "','" . $price . "','" . $dealer_price . "','" . $type_1 . "','1')";
    mysqli_query($con, $query) or die(mysqli_error($con));
    
	// $query = "UPDATE `stock` SET `product_id``type_1`='$type_1',`product_name`='$product_name',`category`='$category',`sub_category`='$sub_category',
	...
	header("Location: add_stock_color.php?product_id=$id");
			...
			<div class="ec-content-wrapper">
				<div class="content">
					<div class="breadcrumb-wrapper d-flex align-items-center justify-content-between">
						<div>
						<h1>Add Stock's For Color Product</h1>
								...
								<div class="card-body">
                                    ...
                                    <table id="responsive-data-table" class="table" style="width:100%">
											...
											<?php
												...
												$query = mysqli_query($con, "SELECT * FROM `product` INNER JOIN `image` ON `product`.`product_id` = `image`.`product_id` WHERE `image`.`product_id` = '$product' ");

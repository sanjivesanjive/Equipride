...
if (isset($_POST['user'])) {
    ...
    // echo "INSERT INTO `register`(`name`, `dob`, `gender`, `email`, `phone`, `password`, `status`) VALUES ('$name','$dob','$gender','$email','$phone','".$password."','1')";
    // die();
    $res = $con->query("INSERT INTO `register`(`userid`,`name`, `dob`, `gender`,`image`, `email`, `phone`, `password`,`address`,`pincode`, `status`) VALUES 
          ('$u_id','$name','$dob','$gender','$fil','$email','$phone','" . $password . "','" . $address . "','$pincode','1')");
    // echo $res;
    // die();
    $login = "INSERT INTO `admin`(`userid`,`email`, `password`,`type`,`status`) VALUES ('$u_id','$email','" . $password . "','user','1')";
  ...
  if (isset($_POST['update'])) {
    ...
    $res = $con->query("UPDATE `register` SET `name`='$name',`dob`='$dob',`gender`='$gender',`image`='$fil',`email`='$email',`pincode` = '$pincode',
					...
					<div class="breadcrumb-wrapper breadcrumb-contacts">
						...
						<div>
							<button type="button" class="btn btn-primary" data-bs-toggle="modal"
								data-bs-target="#modal-add-contact"> Add User
							</button>
						</div>
					</div>
						...
						<div class="col-lg-6 col-xl-4 mb-24px">
							<div class="ec-user-card card card-default p-4">
								...
								<a href="javascript:0" class="media text-secondary">
								  ...
								  ?>
									<!-- <img src="file/<?php echo $q['image'] ?>" class="mr-3 img-fluid" alt="Avatar" style="height: 100px;width: 100px;"> -->
							...
							<div class="modal-content">
								<div class="modal-header justify-content-end border-bottom-0">
                                    <button type="button" class="btn-edit-icon" data-bs-toggle="modal"
												data-bs-target="#modal-add-contact<?php echo $q['id'] ?>"><i class="mdi mdi-pencil"></i>
											</button>
											&nbsp;&nbsp;&nbsp;
									<div class="dropdown">
					...
					<div class="modal fade modal-add-contact" id="modal-add-contact<?php echo $id ?>" tabindex="-1" role="dialog" aria-hidden="true">
						<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                            ...
                            <form class="form-horizontal m-t-30" enctype="multipart/form-data" method="post">
									...
									<?php
									                    $query2=mysqli_query($con,"SELECT * FROM `register` WHERE `id` = '$id'  ") ;
									...
									<div class="modal-body px-4">
										...
										<div class="row mb-2">
											...
											<div class="col-lg-6">
												<div class="form-group mb-4">
													<label for="Birthday">Pincode</label>
													<input type="pincode" class="form-control" id="Birthday" value="<?php echo $q2['pincode'] ?>"
                                                    name="pincode">
												</div>
											</div>
